...
# After Andrej Karpathy "makemore" lecture 3
#
# Runs the full progression from the notebook:
#   1. Baseline (no fix)          -- initial loss ~26, dev NLL ~2.12
#   2. Fix 1: scale W2, zero b2   -- dev NLL ~2.13
#   3. Fix 2: scale W1, b1        -- dev NLL ~2.10
#   4. Kaiming initialization     -- dev NLL ~2.10
#   5. Inline Batch Normalization  -- dev NLL ~2.11
#   6. Deep 6-layer network        -- dev NLL ~2.40 (context bottleneck)
#
# Produces:
...
lr_decay_deep    = 150_000   # LR step for the deep model
...
def run_shallow(C, W1, b1, W2, b2, g, lr_decay_at=None, tag=''):
    ...
    it -- faster and uses less memory than zero_grad()).
...
dev_nll_nofix   = eval_shallow(C, W1, b1, W2, b2, Xdev, Ydev)
print(f'  train NLL: {train_nll_nofix:.4f}  dev NLL: {dev_nll_nofix:.4f}')
...
dev_nll_fix1    = eval_shallow(C, W1, b1, W2, b2, Xdev, Ydev)
print(f'  train NLL: {train_nll_fix1:.4f}  dev NLL: {dev_nll_fix1:.4f}')
...
dev_nll_fix2    = eval_shallow(C, W1, b1, W2, b2, Xdev, Ydev)
print(f'  train NLL: {train_nll_fix2:.4f}  dev NLL: {dev_nll_fix2:.4f}')
...
dev_nll_kaiming    = eval_shallow(C, W1, b1, W2, b2, Xdev, Ydev)
print(f'  train NLL: {train_nll_kaiming:.4f}  dev NLL: {dev_nll_kaiming:.4f}')
...
dev_nll_bn   = eval_shallow_bn(C, W1, W2, b2, bngain, bnbias, bnmean_r, bnstd_r, Xdev, Ydev)
print(f'  train NLL: {train_nll_bn:.4f}  dev NLL: {dev_nll_bn:.4f}')
 
# ---- Summary of shallow model progression ----
 
print('\n---- Shallow model progression ----')
print(f'{"Config":<22}  {"Init Loss":>9}  {"Train NLL":>9}  {"Dev NLL":>9}')
print('-' * 58)
rows = [
    ('No fix',            init_loss_nofix,    train_nll_nofix,    dev_nll_nofix),
    ('Fix 1: W2*0.01',    init_loss_fix1,     train_nll_fix1,     dev_nll_fix1),
    ('Fix 2: W1*0.2',     init_loss_fix2,     train_nll_fix2,     dev_nll_fix2),
    ('Kaiming init',      init_loss_kaiming,  train_nll_kaiming,  dev_nll_kaiming),
    ('Batch Norm',        init_loss_bn,       train_nll_bn,       dev_nll_bn),
]
for name, il, tr, dv in rows:
    print(f'{name:<22}  {il:>9.4f}  {tr:>9.4f}  {dv:>9.4f}')
...
print(f'  Parameters: {n_params_deep:,}')
...
dev_nll_deep   = eval_deep(Xdev, Ydev)
print(f'  Deep model -- train NLL: {train_nll_deep:.4f}  dev NLL: {dev_nll_deep:.4f}')
print('  (train ~= dev means context bottleneck, not overfitting -- model saturated on 3-char context)')
...
# Plot 1: Initialization comparison -- grouped bars, initial loss vs final dev NLL
# The no-fix initial loss (~26) towers over the rest, making the contrast
# between bad and good initialization immediately visible.
config_names = ['No Fix', 'Fix 1\nW2*0.01', 'Fix 2\nW1*0.2', 'Kaiming\nInit', 'Batch\nNorm']
init_losses  = [r[1] for r in rows]
dev_nlls     = [r[3] for r in rows]
 
x_pos = np.arange(len(config_names))
width = 0.35
 
fig, ax = plt.subplots(figsize=(10, 5))
bars1 = ax.bar(x_pos - width/2, init_losses, width, label='Initial loss',   color='steelblue',  alpha=0.8)
bars2 = ax.bar(x_pos + width/2, dev_nlls,    width, label='Final Dev NLL',  color='darkorange', alpha=0.8)
ax.set_xticks(x_pos)
...
# Plot 2: Deep model training loss -- raw (light) + smoothed (bold)
lossi_deep_smooth = torch.tensor(lossi_deep).view(-1, 1000).mean(1)
stepi_deep_smooth = torch.arange(500, max_iters, 1000)
 
fig, ax = plt.subplots(figsize=(10, 4.5))
ax.plot(range(max_iters), lossi_deep, alpha=0.15, color='steelblue', linewidth=0.5, label='raw')
ax.plot(stepi_deep_smooth.tolist(), lossi_deep_smooth.tolist(), color='steelblue', linewidth=2, label='smoothed (1k-step mean)')
ax.axvline(x=lr_decay_deep, color='crimson', linestyle='--', linewidth=1,
...
# Plots 3-4: weight grad ratios and update ratios
...
with open('outputs/generated_names/03_deep_network_generated.txt', 'w') as f:
    f.write(f'Model    : 6-layer deep MLP with BatchNorm\n')
    f.write(f'Train NLL: {train_nll_deep:.4f}  |  Dev NLL: {dev_nll_deep:.4f}  |  Parameters: {n_params_deep:,}\n')
    f.write('--------------------------------\n')
    for name in names:
        f.write(name + '\n')
